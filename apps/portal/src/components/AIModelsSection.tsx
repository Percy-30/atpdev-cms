...
export default function AIModelsSection({ projects, ui, aiModels }: { projects: any[], ui: any, aiModels: AIModelData[] }) {
  ...
  return (
    <section id="ai-models" className="py-24 relative overflow-hidden bg-[#07080a] border-t border-gray-900">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activeModels.map((model, i) => (
            <motion.div
              ...
              className="bg-[#12141a]/80 backdrop-blur-md border border-gray-800 hover:border-gray-600 rounded-3xl p-8 transition-all group hover:-translate-y-1 hover:shadow-2xl hover:shadow-black/50"
            >
              ...
              <p className="text-xs text-gray-500 mb-4">{model.provider}</p>
              <p className="text-sm text-gray-400 leading-relaxed mb-6">
                {model.description}
              </p>

              <div className="space-y-3 mt-auto">
                <div className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">{ui.aiCaps}</div>
                <div className="flex flex-wrap gap-2">
                  {model.capabilities.slice(0, 3).map(cap => (
                    <span key={cap} className="inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-lg bg-gray-800/50 text-gray-300 border border-gray-700/50">
