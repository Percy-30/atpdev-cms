...
export default function HomeClient({
  ...
  useEffect(() => {
    ...
    const translateAll = async () => {
      // Translate projects
      const translatedProjects = await Promise.all(initialProjects.map(async p => ({
        ...p,
      ...
      // Translate config
      if (initialConfig) {
        const tw = initialConfig.hero_typewriter || [];
        setConfig({
          ...initialConfig,
          hero_title: await translateClient(initialConfig.hero_title, lang),
          hero_subtitle: await translateClient(initialConfig.hero_subtitle, lang),
          bio_short: await translateClient(initialConfig.bio_short, lang),
          hero_typewriter: await Promise.all(tw.map((w: string) => translateClient(w, lang))),
        });
      }
      ...
      <section id="portfolio" className="py-20 relative bg-[#0b0c10] border-t border-gray-900">
        <div className="max-w-6xl mx-auto">
          <div className="mb-16 text-center">
            <h2 className="text-3xl md:text-4xl font-extrabold text-white mb-4">{ui.featuredProjects}</h2>
                ...
                return (
                  <div>
                    <div className="relative h-72 bg-black flex items-center justify-center">
                      ...
                      <button onClick={() => setSelectedProject(null)} className="absolute top-4 right-4 bg-black/50 hover:bg-black/80 text-white p-2 rounded-full transition-colors border border-gray-600">
                        <X size={20} />
                      </button>
                    </div>
                    <div className="p-8 md:p-10">
                      ...
                      <p className="text-gray-300 text-lg mb-8 leading-relaxed">{p.description}</p>
                      <div className="mb-10">
                        <h4 className="text-sm text-gray-500 uppercase tracking-widest font-bold mb-4">{ui.techStackTitle}</h4>
                        <div className="flex flex-wrap gap-3">
                          {p.stack.map(tech => (
                            <span key={tech} className="bg-gray-800 border border-gray-700 text-gray-200 px-4 py-2 rounded-lg text-sm font-medium">{tech}</span>
                          ))}
                        </div>
                      </div>
      ...
      <footer className="bg-[#050608] border-t border-gray-900 pt-16 pb-8">
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-10 mb-12">
          <div>
            ...
            <p className="text-gray-400 text-sm leading-relaxed max-w-xs">{ui.footerBio}</p>
          </div>
          <div>
