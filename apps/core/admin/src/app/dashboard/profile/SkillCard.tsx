...
export default function SkillCard({ skill, iconOptions, colorOptions, editAction, removeAction, toggleAction }: Props) {
  ...
  return (
    <div className={`bg-\[\#262626\] border rounded-2xl overflow-hidden transition-all ${skill.is_active ? "border-gray-800" : "border-gray-800/30 opacity-50"}`}>
                ...
                {colorOptions.map(o => {
                  ...
                  return (
                    <label key={o.key} className="cursor-pointer group relative" title={o.label}>
                      <input type="radio" name="color_key" value={o.key} defaultChecked={o.key === skill.color_key} className="peer sr-only" />
                      <div className={`w-5 h-5 rounded-full border-2 border-transparent peer-checked:border-white ${bgMap[o.key] || 'bg-gray-500'} transition-all group-hover:scale-110 shadow-sm peer-checked:scale-110 peer-checked:shadow-[0_0_10px_rgba(255,255,255,0.3)]`}></div>
                    </label>
                  );
                })}
              </div>
