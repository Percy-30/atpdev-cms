            ...
            <div key={c.label} className="flex items-center justify-between group">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => c.setVal(e.target.value.toUpperCase())}
                  disabled={colorTheme !== "custom"}
                  className="font-mono text-xs text-gray-400 bg-[#111111] px-2 py-1 rounded-md border border-gray-800 focus:outline-none focus:border-blue-500 disabled:opacity-80 w-[72px]"
                />
              </div>
            </div>
          ))}
